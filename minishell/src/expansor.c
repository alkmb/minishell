...
/*   Updated: 2024/02/04 23:29:18 by akambou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int *is_malloced, char *var_name)
{
	...
	if (var_value != NULL)
	{
		new_command = malloc((ft_strlen(commands[i]) - var_name_len + ft_strlen(var_value)+ 2) * sizeof(int));
		ft_strlcpy(new_command, commands[i], *j);
		ft_strlcpy(new_command + *j - var_name_len - 1, var_value, ft_strlen(var_value)+ 2);
		ft_strlcpy(new_command + *j - var_name_len - 1 + ft_strlen(var_value), commands[i] + *j, ft_strlen(commands[i] + *j) + 1);
